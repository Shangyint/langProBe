...
def _maybe_assure_num_tasks_per_scenario(
    ...
    if num_tasks_per_scenario is not None:
        ...
        for generator_id, task_ids_ in generator_id_to_task_ids.items():
            ...
            task_numbers_found = sorted(
                task_id_to_number(task_id_) for task_id_ in task_ids_
            )
...
def load_task_ids(
    ...
    if only_tagged is not None:
        task_ids = [
            task_id for task_id in task_ids if task_id_to_tag(task_id) in only_tagged
        ]
    ...
    if difficulty is not None:
        ...
        def get_task_difficulty(task_id: str) -> int:
            task_metadata_file_path = (
                tasks_directory / task_id / "ground_truth" / "metadata.json"
            )
        ...
        task_ids = [
            task_id
            for task_id in task_ids
            if get_task_difficulty(task_id) == difficulty
        ]
    if num_tasks_per_scenario is not None:
        task_ids = [
            task_id
            for task_id in task_ids
            if task_id_to_number(task_id) <= num_tasks_per_scenario
        ]
