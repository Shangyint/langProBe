...
class AppWorldBench(Benchmark):
    def init_dataset(self):
        ...
        self.dev_set = [
            dspy.Example(task_id=task_id).with_inputs("task_id")
        ...
        unique_trainval_scenario_ids = list(
            set([task_id.split("_")[0] for task_id in trainval_task_ids])
        )
        rng = random.Random(0)
        ...
        self.train_set = [
            dspy.Example(task_id=task_id).with_inputs("task_id")
            for task_id in trainval_task_ids
            if task_id.split("_")[0] in train_scenario_ids
        ]
        self.val_set = [
            dspy.Example(task_id=task_id).with_inputs("task_id")
            for task_id in trainval_task_ids
            if task_id.split("_")[0] in val_scenario_ids
        ]
